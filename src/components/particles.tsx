...
export default function Particles({
    ...
    const drawCircle = (circle: Circle, update = false) => {
        if (context.current) {
            const { x, y, translateX, translateY, size, alpha, color, shape, rotation } = circle;
            ...
            switch (shape) {
                case 'square':
                    ...
                    break;
                case 'triangle':
                    context.current.moveTo(x, y - size / 2);
                    context.current.lineTo(x - size / 2, y + size / 2);
                    context.current.lineTo(x + size / 2, y + size / 2);
    ...
    const animate = () => {
        ...
        circles.current.forEach((circle: Circle, i: number) => {
            ...
            circle.size += Math.sin(circle.pulsation) * circle.pulsationSpeed;
            circle.pulsation += 0.1;
